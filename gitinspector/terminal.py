...
def printb(string):
    logging.info(__bold__ + string + __normal__)
...
def check_terminal_encoding():
    if sys.stdout.isatty() and (sys.stdout.encoding == None or sys.stdin.encoding == None):
        logging.info(_("WARNING: The terminal encoding is not correctly configured. gitinspector might malfunction. "
...
def output_progress(text, pos, length):
    if sys.stdout.isatty():
        ...
        logging.info("\r{0}\r{1}".format(" " * width, progress_text), end="")
        sys.stdout.flush()
