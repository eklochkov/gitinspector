...
def __output_row__text__(timeline_data, periods, names):
    ...
    for period in periods:
        logging.info(terminal.rjust(period, 10))

    logging.info(terminal.__normal__)

    for name in names:
        if timeline_data.is_author_in_periods(periods, name[0]):
            logging.info(terminal.ljust(name[0], 20)[0:20 - terminal.get_excess_column_count(name[0])])
    ...
    logging.info(terminal.__bold__ + terminal.ljust(_(MODIFIED_ROWS_TEXT), 20) + terminal.__normal__)
    ...
    for period in periods:
        ...
        logging.info(terminal.rjust(total_changes, 10))
