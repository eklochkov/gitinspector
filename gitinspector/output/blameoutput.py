...
class BlameOutput(Outputable):
    ...
    def output_text(self):
        ...
        logging.info(textwrap.fill(_(BLAME_INFO_TEXT) + ":", width=terminal.get_size()[0]) + "\n")
        terminal.printb(
        ...
        for i in sorted(self.blame.get_summed_blames().items()):
            logging.info(terminal.ljust(i[0], 20)[0:20 - terminal.get_excess_column_count(i[0])])
            logging.info(str(i[1].rows).rjust(10))
            logging.info("{0:.1f}".format(Blame.get_stability(i[0], i[1].rows, self.changes)).rjust(14))
            logging.info("{0:.1f}".format(float(i[1].skew) / i[1].rows).rjust(12))
            logging.info("{0:.2f}".format(100.0 * i[1].comments / i[1].rows).rjust(19))
